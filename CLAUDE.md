...
- Replacing that sleep with adaptive polling does NOT help (measured 17.4s vs 17.0s) — the only lever is walking fewer passes. `_SCROLL_IDLE_PIXELS` (40000) stops the walk after that much scrolling with no new image; strips run ~20-25k px each, so it leaves over a full strip of slack. Collect + measure + scroll are one `execute_script` per pass, since three round trips cost more over the WebDriver wire than the settle they bracket
